    ...
    <MobileShell>
      <header className="sticky top-0 z-20 border-b border-zinc-100 bg-white/95 px-5 py-4 backdrop-blur">
        <div className="flex items-center gap-3">
          <Link href="/" aria-label="Back home" className="grid size-9 place-items-center rounded-full bg-zinc-100 text-zinc-700">
            <ChevronLeft className="size-5" />
          </Link>
          <Avatar src={user.avatarUrl} alt={user.username} size="md" ring="active" />
          <div className="min-w-0">
            <h1 className="truncate text-2xl font-black leading-tight">{user.username}</h1>
            <p className="truncate text-sm font-bold text-zinc-500">{user.name}</p>
          </div>
        </div>
      </header>
      ...
      <section className="px-5 py-6">
        <div className="flex items-start gap-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 truncate text-lg font-bold text-zinc-500">@{user.username}</p>
        ...
        <div className="mt-6 grid grid-cols-4 rounded-lg border border-zinc-100 bg-zinc-50 p-4 text-center">
          ...
          <div>
            <p className="text-2xl font-black">{formatCompactNumber(stats.products)}</p>
      ...
      <PublicProfileTabs posts={posts} products={products} initialTab={tab === "products" ? "products" : "posts"} />
      <SheetHost />
    </MobileShell>
  );
}
