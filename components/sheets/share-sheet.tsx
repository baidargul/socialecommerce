...
export function ShareSheet({ open }: { open: boolean }) {
  ...
  async function nativeShare() {
    if (navigator.share) {
      await navigator.share({ title: "Social Commerce", text: shareText, url: `/post/${selectedPost!.slug}` });
      incrementShares(selectedPost!.id);
    } else {
      await copyShareText();
    }
  ...
  return (
    <Sheet open={open} onClose={closeSheet} className="min-h-[54dvh] px-0">
      <div className="flex items-center justify-between px-8">
        ...
        <IconButton label="Close share sheet" icon={<X className="size-9" />} onClick={closeSheet} />
      </div>
      ...
      <div className="mx-8 mt-8 flex items-center gap-5 rounded-2xl bg-zinc-100 p-6">
        ...
        <IconButton label="Copy share text" icon={<Copy className="size-8" />} onClick={copyShareText} />
      </div>

      <div className="mt-8 border-y border-zinc-200 px-8 py-6">
        <div className="flex gap-8 overflow-x-auto">
          {demoUsers.slice(1, 4).map((user) => (
            <button key={user.id} className="grid w-24 shrink-0 justify-items-center gap-2 text-center text-lg leading-tight">
      ...
      <div className="mt-8 flex gap-8 overflow-x-auto px-8">
        {[
          ...
          return (
            <button key={option.label} className="grid w-24 shrink-0 justify-items-center gap-3 text-center text-lg" onClick={option.action}>
