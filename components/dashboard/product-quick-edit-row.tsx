...
function initialDraft(product: Product, categories: CategoryItem[]): QuickEditDraft {
...
export function ProductQuickEditRow({ product, categories, colSpan, onCancel }: ProductQuickEditRowProps) {
  const router = useRouter();
  const [draft, setDraft] = useState<QuickEditDraft>(() => initialDraft(product, categories));
  ...
  function update<K extends keyof QuickEditDraft>(key: K, value: QuickEditDraft[K]) {
      ...
      <td colSpan={colSpan} className="px-4 py-4">
        <form onSubmit={submitQuickEdit} className="grid gap-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="text-sm font-black text-[#d62976]">{formatPrice(price > 0 ? price : 0)}</div>
          </div>

          <div className="grid gap-3 lg:grid-cols-4">
            <Field label="Name">
              <TextField value={draft.name} onChange={(event) => update("name", event.target.value)} required />
            </Field>
            <Field label="Slug">
              <TextField value={draft.slug} onChange={(event) => update("slug", event.target.value)} placeholder={generatedSlug} />
            </Field>
            <SearchableDropdown
              label="Category"
            ...
            />
            <Field label="SKU">
              <TextField value={draft.sku} onChange={(event) => update("sku", event.target.value)} />
            </Field>
          </div>

          <div className="grid gap-3 lg:grid-cols-4">
            <Field label="Original price">
              <TextField value={draft.originalPrice} onChange={(event) => updateOriginalPrice(event.target.value)} type="number" min="0.01" step="0.01" />
            </Field>
            <Field label="Discount %">
              <TextField value={draft.discountPercent} onChange={(event) => updateDiscount(event.target.value)} type="number" min="0" max="95" />
            </Field>
            <Field label="Sale price">
              <TextField value={draft.price} onChange={(event) => updatePrice(event.target.value)} required type="number" min="0.01" step="0.01" />
            </Field>
            <Field label="Stock">
          ...
          <div className="grid gap-2">
            <Field label="Inventory tags">
              <div className="flex gap-2">
                <TextField value={tagInput} onChange={(event) => setTagInput(event.target.value)} onKeyDown={onTagKeyDown} placeholder="Type tag and press Enter" />
                <button type="button" onClick={() => addTag()} className="h-9 rounded border border-zinc-200 bg-white px-3 text-xs font-black text-zinc-700">
                  Add
                </button>
              </div>
            </Field>
            {draft.tags.length ? (
              <div className="flex flex-wrap gap-2">
                {draft.tags.map((tag) => (
                  <span key={tag} className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-1 text-xs font-black text-[#d62976]">
                    {tag}
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={onCancel} className="inline-flex min-h-10 items-center gap-2 rounded border border-zinc-200 bg-white px-4 text-sm font-black text-zinc-700">
              <X className="size-4" />
              Cancel
            </button>
            <button disabled={saving} className="inline-flex min-h-10 items-center gap-2 rounded bg-[#d62976] px-4 text-sm font-black text-white disabled:opacity-60">
